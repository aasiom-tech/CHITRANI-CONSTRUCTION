  ...
  return (
    <>
      ...
      >
        {/* Inner Visual Shell */}
        <div
          className={`w-full bg-white text-[#2D2D2D] border border-[#E7E7E7] transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] flex items-center justify-between px-4 sm:px-6 lg:px-8 ${
            isExpanded
              ? 'h-[84px] sm:h-[88px] rounded-[18px] shadow-[0_10px_30px_rgba(45,45,45,0.06)]'
              : 'h-[62px] sm:h-[66px] rounded-none rounded-b-[16px] shadow-[0_4px_16px_rgba(45,45,45,0.06)]'
          }`}
        >
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="lg:hidden p-2 rounded-[10px] bg-[#F9F7F2] hover:bg-[#FFB300] text-[#2D2D2D] transition-colors border border-[#E7E7E7] active:scale-95 focus:outline-hidden focus:ring-2 focus:ring-[#FFB300] min-h-[44px] min-w-[44px] flex items-center justify-center"
            >
            ...
            >
              ...
              <div
                className={`bg-[#FFB300] text-[#2D2D2D] flex items-center justify-center font-bold rounded-[12px] shadow-xs transition-all duration-260 ease-in-out shrink-0 ${
                  isExpanded
                    ? 'w-10 h-10 opacity-100 scale-100'
                    : 'w-8 h-8 lg:w-0 lg:h-0 lg:opacity-0 lg:scale-90 overflow-hidden'
                }`}
              >
                <HardHat className="w-5 h-5 text-[#2D2D2D]" />
              </div>

              {/* Company Title & Subtitle */}
              <div className="flex flex-col justify-center">
                <span
                  className={`font-heading font-bold tracking-tight text-[#2D2D2D] group-hover:text-[#FFB300] transition-colors uppercase leading-none ${
                    isExpanded ? 'text-base sm:text-lg' : 'text-sm sm:text-base'
                  }`}
                >
                  CHITRANI <span className="text-[#FFB300]">CONSTRUCTION</span>
                </span>
                ...
                >
                  <span className="font-body text-[10px] sm:text-[11px] text-[#5D5D5D] block font-medium tracking-wider leading-none uppercase">
          ...
          <nav
            ...
            className="hidden lg:flex items-center gap-5 xl:gap-7 text-xs font-heading font-semibold uppercase tracking-wider text-[#2D2D2D]"
          >
            {navItems
                ...
                return (
                  <Link
                    ...
                    className={`relative py-1.5 transition-colors duration-180 ${
                      isActive ? 'text-[#2D2D2D] font-bold' : 'text-[#2D2D2D] hover:text-[#FFB300]'
                    }`}
                  >
                    <span>{item.label}</span>
                    {isActive && (
                      <span className="absolute bottom-0 left-0 w-full h-[2.5px] bg-[#FFB300] rounded-full animate-fade-in" />
                    )}
                  </Link>
                );
              })}
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/request-quote"
              className={`hidden lg:inline-flex items-center justify-center gap-2 bg-[#FFB300] hover:bg-[#E59A00] text-[#2D2D2D] font-heading font-bold uppercase tracking-wider transition-all duration-250 shadow-xs hover:-translate-y-0.5 active:translate-y-0 focus:outline-hidden focus:ring-2 focus:ring-[#FFB300] ${
                isExpanded
            ...
            <a
              ...
              className="lg:hidden flex items-center gap-1.5 px-3.5 py-2 rounded-[10px] bg-[#FFB300] hover:bg-[#E59A00] text-[#2D2D2D] font-heading text-xs font-bold uppercase shadow-xs transition-all active:scale-95 min-h-[44px]"
            >
